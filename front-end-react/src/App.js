...
const App = () => {
  ...
  const fetchUserData = async () => {
    try {
      const response = await fetch('http://user-service.microapp-deploy.svc.cluster.local:3001/user');
  ...
  const fetchOrderData = async () => {
    try {
      const response = await fetch('http://order-service.microapp-deploy.svc.cluster.local:3002/order');
      const data = await response.json();
